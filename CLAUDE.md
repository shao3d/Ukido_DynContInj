...
### Органичная интеграция CTA через few-shot обучение
**Проблема:** Claude пытается встроить CTA, но делает это механически (отдельные предложения)

**Решение:** Добавить few-shot примеры в промпт response_generator.py
```python
# В _build_messages() добавить примеры органичной интеграции:
few_shot_examples = [
    {"role": "user", "content": "Мой ребёнок боится новых мест"},
    {"role": "assistant", "content": "...и чтобы ваш ребёнок мог попробовать без стресса, первое занятие у нас бесплатное."},
    # НЕ отдельным блоком, а естественным завершением мысли
]
```

**Где добавлять:** 
- response_generator.py в метод _build_messages()
- После system prompt, перед историей диалога
- 2-3 примера для каждого user_signal

**Ожидаемый результат:**
- CTA будет встраиваться как естественное завершение мысли
- Не будет резких переходов типа "Кстати, первое занятие..."
- Повысится конверсия за счёт органичности
