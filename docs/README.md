# Project documentation

Use these documents as the current source of truth:

- [Architecture](architecture.md) — runtime components, request flow and data
  boundaries.
- [API](API.md) — public and administrative HTTP/SSE contracts.
- [Deployment](deployment-beyondhorizon.md) — production, release and rollback
  procedure.

The root [README](../README.md) contains local setup and the shortest project
overview. Product facts used by the chatbot live in `data/documents/` and
`data/documents_compressed/`; they are runtime content, not engineering docs.

Superseded plans, research and generated reports remain available in Git
history instead of the active documentation tree.
