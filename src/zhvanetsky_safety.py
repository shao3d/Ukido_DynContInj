...
class SafetyChecker:
    ...
    BLACKLIST_PATTERNS = [
        # Здоровье и болезни
        r'\b(болезн|болеет|болею|заболел|больниц|врач|лечен|операци|диагноз|лекарств|таблетк|укол|скорая|симптом|температура|грипп|ковид|covid|вирус)\b',
        
        # Трагедии и несчастья
        r'\b(умер|погиб|смерт|похорон|кладбищ|могил|траге|авари|катастроф|несчастн|покойн|усоп|скончал)\b',
        
        # Война и насилие
        r'\b(войн|бомб|оружи|убив|убий|стрел|взрыв|террор|напад|атак|ракет|снаряд|окоп|фронт)\b',
        
        # Финансовые проблемы
        r'\b(банкрот|долг|кредит|нет денег|безработ|уволен|нищ|бедн|голод|коллектор|просрочк)\b',
        
        # Семейные проблемы
        r'\b(развод|расста|изменя|предател|ссор|сканда|алимент|раздел|бросил|ушел от|ушла от)',
