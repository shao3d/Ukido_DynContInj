...
СПЕЦИАЛЬНЫЕ ПРАВИЛА ДЛЯ СОЦИАЛЬНЫХ БЕЗ БИЗНЕС-КОНТЕНТА:
Чисто социальные интенты БЕЗ вопросов о школе → status: "offtopic" + social_context:
...
НО! Если есть ЛЮБОЙ бизнес-контент после социального маркера → success:
- "Спасибо, запишите нас" → success + user_signal: "ready_to_buy" + social_context: "thanks"
- "Привет! Сколько стоит?" → success + documents: ["pricing.md"] + social_context: "greeting"
- "Спасибо. Мы согласны." → success + user_signal: "ready_to_buy" + social_context: "thanks"
- "Благодарю! Регистрируйте меня" → success + user_signal: "ready_to_buy" + social_context: "thanks"
- "Спасибо. Действуем." → success + user_signal: "ready_to_buy" + social_context: "thanks"

КРИТИЧЕСКОЕ ПРАВИЛО: Любое упоминание ЗАПИСИ, РЕГИСТРАЦИИ, СОГЛАСИЯ, ДЕЙСТВИЙ после социального слова = ready_to_buy!
...
ПРАВИЛО IMPLICIT QUESTIONS для ready_to_buy:
Если user_signal = ready_to_buy И decomposed_questions пустой:
ОБЯЗАТЕЛЬНО добавь уточняющий вопрос в decomposed_questions!
Примеры:
- Определил ready_to_buy для "Мы согласны" → добавь ["На что именно вы согласны - записаться на курс или нужны детали?"]
- Определил ready_to_buy для "Действуем" → добавь ["Какие конкретные шаги вы хотите предпринять?"]
- Определил ready_to_buy для "Запишите" → добавь ["Как оформить запись на курс?"]
Это критично для правильной генерации ответа!
...
ready_to_buy: "запишите нас", "когда старт?", "онлайн есть?", "trial?", "Спасибо, запишите нас", "Благодарю! Регистрируйте", "Мы согласны", "Действуем"
