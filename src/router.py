...
- Скептические вопросы и критика концепции классифицируй как success:
  * "это не секта?" → success с ukido_philosophy.md
  * "не верю в soft skills" → success с methodology.md, student_results.md
  * "это НЕ работает" → success с student_results.md, roi_and_future.md
  * "онлайн - это НЕ обучение" → success с online_format_benefits.md
  * "не нужны навыки общения" → success с roi_and_future.md
...
КРИТИКА = ВОПРОС! Любая критика содержит скрытый вопрос:
- "Не нужны навыки общения" → ["Зачем детям навыки общения?"]
- "Это НЕ работает" → ["Какие доказательства эффективности?"]
- "Лучше бы математику" → ["Чем soft skills важнее математики?"]
...
ВАЖНО: Распознавание непонимания формата обучения
Если родитель упоминает "забирать", "привозить", "довозить" - это implicit вопрос о необходимости физического присутствия:
- "После работы забирать буду" → implicit: ["Нужно ли приезжать за ребёнком после занятий?"]
- "Кто будет привозить?" → implicit: ["Занятия проходят очно или онлайн?"]
- "Далеко от нас, как довозить?" → implicit: ["Нужно ли физически присутствовать на занятиях?"]
- "Во сколько забрать ребёнка?" → implicit: ["В какое время заканчиваются занятия и нужно ли забирать?"]
...
ИСКЛЮЧЕНИЕ ДЛЯ КРИТИКИ: Критика концепции soft skills или онлайн-обучения → success!
- "Не верю в soft skills" → success (implicit: "Почему soft skills важны?")
- "Онлайн - это НЕ обучение" → success (implicit: "Какие преимущества онлайн?")
- "Не нужны навыки общения" → success (implicit: "Зачем нужны навыки общения?")
- "Это НЕ работает" → success (implicit: "Какие результаты у выпускников?")
