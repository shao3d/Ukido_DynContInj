        ...
        try:
            ...
            try:
                ...
                if result["status"] in valid_signals and result["status"] not in valid_statuses:
                    ...
                    result["status"] = "success"  # По умолчанию success для корпоративных клиентов
...
Пример 1:
Запрос: "Привет! Сколько стоит курс для 10-летнего и есть ли у вас скидки?"
Декомпозиция:
1. "Сколько стоит курс?"
2. "Подходит ли курс для 10-летнего ребёнка?"
3. "Какие есть скидки?"
(приветствие проигнорировано)

ВАЖНО: Объединяй связанные вопросы!
Пример объединения:
Запрос: "У меня двое детей 8 и 12 лет, младший стеснительный, старший гиперактивный, какие курсы подойдут?"
НЕПРАВИЛЬНО (4 вопроса):
1. Какие курсы для 8 лет?
2. Какие курсы для 12 лет?  
3. Подойдет стеснительному?
4. Подойдет гиперактивному?

ПРАВИЛЬНО (2 вопроса):
1. "Какие курсы подойдут для стеснительного ребенка 8 лет и гиперактивного 12 лет?"
2. (если был вопрос про скидки, он отдельно)

Пример 2:
Запрос: "Ребёнок очень стеснительный, боюсь не справится. И ещё - а если не понравится?"
Декомпозиция:
1. "Подходит ли обучение для стеснительного ребёнка?"
2. "Как вы работаете с застенчивыми детьми?"
3. "Можно ли вернуть деньги если не понравится?"

Пример 3 (с контекстом):
История: обсуждали пробный урок
Запрос: "А это платно? И что дальше?"
Декомпозиция:
1. "Пробный урок платный?"
2. "Что происходит после пробного урока?"

Пример 4 (ультра-краткие контекстуальные вопросы):
История: User: "Есть ли у вас курсы для подростков?" Assistant: "Да, у нас есть курсы..."
Запрос: "А?"
Декомпозиция:
1. "Какие именно курсы для подростков есть в школе?"
(ВАЖНО: "А?" - это просьба продолжить/уточнить предыдущий ответ)

Пример 4.1 (А? после цен):
История: Assistant: "...цена курса 6000 грн в месяц..."
Запрос: "А?"
Декомпозиция:
1. "Расскажите подробнее о ценах и скидках"
(НЕ offtopic! Пользователь хочет больше информации о ценах)

Пример 5 (краткий вопрос о продолжении):
История: Assistant рассказал о преимуществах курса
Запрос: "И всё?"
Декомпозиция:
1. "Какие еще преимущества есть у курса?"
2. "Что еще важно знать о курсе?"
(ВАЖНО: "И всё?" = пользователь ждет больше информации)

Пример 6 (вопрос о следующих шагах):
История: обсуждали результаты обучения
Запрос: "А дальше?"
Декомпозиция:
1. "Что происходит после окончания курса?"
2. "Какие дальнейшие возможности развития?"
(ВАЖНО: "А дальше?" = вопрос о следующих этапах/последствиях)

Пример 7 (неясная краткая реплика):
История: говорили о цене курса "Эмоциональный Компас"
Запрос: "И что это даёт?"
Декомпозиция:
1. "Какие результаты даёт курс 'Эмоциональный Компас'?"
2. "Какая польза от курса 'Эмоциональный Компас'?"
(ВАЖНО: используй контекст истории чтобы понять о чём "это")

Пример 8:
Запрос: "У вас есть курсы? Какая погода сегодня? И кто преподаватели?"
Декомпозиция:
1. "Какие курсы есть в школе?"
2. "Кто преподаватели?"
(вопрос про погоду - оффтопик, игнорируем)

Пример 9:
Запрос: "Наверное дорого... А пробный урок есть?"
Декомпозиция:
1. "Какая цена на курсы?"
2. "Есть ли пробный урок?"

Пример 10 (слишком много вопросов):
Запрос: "Сколько стоит 'Капитан Проектов', кто ведет, сколько детей в группе и какое расписание?"
Декомпозиция:
1. "Сколько стоит курс 'Капитан Проектов'?"
2. "Кто ведет курс 'Капитан Проектов'?"
3. "Сколько детей в группе курса 'Капитан Проектов'?"
4. "Какое расписание курса 'Капитан Проектов'?"
(Внимание: 4 вопроса - будет need_simplification!)

СОЦИАЛЬНЫЙ КОНТЕКСТ:
Если в начале запроса есть социальные элементы (привет, спасибо, извините, до свидания и т.д.),
НЕ включай их в декомпозированные вопросы, но отметь в поле "social_context":
- "greeting" - приветствие
- "thanks" - благодарность  
- "apology" - извинение
- "farewell" - прощание

Примеры с социальным контекстом:
"Привет! Сколько стоит курс?" → social_context: "greeting", вопросы: ["Сколько стоит курс?"]
"Спасибо! А еще расскажите про учителей" → social_context: "thanks", вопросы: ["Расскажите про учителей"]
"Извините, а для какого возраста?" → social_context: "apology", вопросы: ["Для какого возраста курсы?"]
"До свидания!" → social_context: "farewell", вопросы: []
...
- «Отправьте договор/ссылку», «Дайте контакт/почту», «Хочу записаться», «Как записаться»,
- Корпоративные/юр. вопросы: «работаете с НДС?», «с кем подписывается договор?», «пришлите проект договора»,
  трактуй как школьные (это операционные запросы об обучении) → faq.md, conditions.md
...
- КОРПОРАТИВНЫЕ КЛИЕНТЫ (всегда ready_to_buy):
  * "работаете с юр.лицами", "юрлицо", "компания", "для сотрудников"
  * "НДС? Договор?", "Акты?", "КП на почту", "коммерческое предложение"
  * Официальный стиль: "Добрый день", "С уважением", hr@, @company
  * Краткие деловые вопросы: "10 детей. Скидка?", "Сроки? Старт?"
...
- "Что за развод для родителей? Сколько стоит?" → "price_sensitive" (агрессия + цена)
- "30 тысяч?! Вы с ума сошли?" → "price_sensitive" (шок от цены)
- "Реально работает или бабло тянете?" → "price_sensitive" (недоверие к ценности)
- "Покажите реальные отзывы" (после вопросов о цене) → "price_sensitive" (проверка ценности)
- "Гарантии какие?" (после агрессивных вопросов) → "price_sensitive" (продолжение скепсиса)
...
- "Курс лидерства для 13 лет. Когда старт?" → "ready_to_buy" (краткий стиль + логистика)
- "Онлайн есть?" → "ready_to_buy" (проверка формата перед записью)
- "Запишите нас" → "ready_to_buy" (прямая команда к действию)
- "Работаете с юр.лицами?" → "ready_to_buy" (корпоративный клиент)
- "НДС? Договор?" → "ready_to_buy" (корпоративные документы)
- "10 детей. Скидка?" → "ready_to_buy" (массовая закупка)
- "КП на почту: hr@company" → "ready_to_buy" (запрос коммерческого предложения)
- "У меня трое: 7, 10 и 12 лет. Скидки?" → "price_sensitive" (многодетная, начинает со скидок)
- "Всех троих сразу?" → "price_sensitive" (продолжает про скидки)
- "Субботы подходят. Записывайте всех" → "ready_to_buy" (переход к записи)
- "Что за вайб у школы?" → "exploring_only" (блогер изучает)
- "Trial есть?" → "ready_to_buy" (блогер готов попробовать)
- "Book us на пробное!" → "ready_to_buy" (прямая запись)
