...
OFFERS_CATALOG = {
    ...
    "exploring_only": {
        "text": "📞 Если у вас остались вопросы, можем провести бесплатную 15-минутную консультацию. Это поможет понять, подойдут ли наши курсы вашему ребенку.",
        "priority": "medium",  # Повышаем приоритет для большей персонализации
        "placement": "end"
    }
}
...
User: В наше время без курсов росли. Зачем это нужно?
Assistant: Мир изменился. Сегодня 65% профессий требуют навыков коммуникации и работы в команде. Дети, развивающие soft skills с раннего возраста, показывают на 40% лучшие результаты в учебе. Наша методика основана на практике - дети учатся через игры и проекты, а не через скучные лекции. За 3 года работы мы помогли 542 детям стать увереннее. Это не замена традиционному образованию, а важное дополнение для успеха в современном мире.

User: Что за вайб у вашей школы?
Assistant: У нас дружелюбная и поддерживающая атмосфера. Никакого давления или оценок - только позитивное подкрепление. Дети работают в мини-группах до 8 человек, где каждый чувствует себя важным. Используем современные методики: геймификацию, проектный подход, арт-терапию. Преподаватели - молодые профессионалы, которые говорят с детьми на одном языке. У нас есть собственная система мотивации с бейджами и достижениями. Родители отмечают, что дети ждут наших занятий с нетерпением.
"""
}
