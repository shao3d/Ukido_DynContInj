...
OFFERS_CATALOG = {
    ...
    "exploring_only": None  # Без дополнительного предложения для общих вопросов
}
...
Assistant: Ukido - это школа развития soft skills для детей 7-14 лет. Мы помогаем детям стать увереннее, научиться выступать публично, управлять эмоциями и работать в команде. У нас 3 основных курса: "Юный Оратор" для развития навыков презентации, "Эмоциональный Компас" для управления эмоциями, и "Капитан Проектов" для развития лидерских качеств. Занятия проходят в мини-группах до 8 человек, что позволяет уделить внимание каждому ребенку.
"""
}
