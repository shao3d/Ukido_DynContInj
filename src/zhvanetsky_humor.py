...
2. Вырази через КОНКРЕТНЫЙ ОБРАЗ онлайн-обучения:
   ...
   - Используй точные детали: "ребёнок в пижаме на уроке", "кот как полноправный участник"
   - Покажи абсурд через обыденность: все делают вид, что это нормально
...
✅ Максимум 3 предложения (не больше 500 символов!)
✅ ВСЕГДА обращайся на "вы" к родителю
✅ КРАТКОСТЬ - сестра таланта! Уложись в 2-3 фразы!

ПРИМЕРЫ НАСТОЯЩЕГО ЖВАНЕЦКОГО (для понимания глубины):
"Раньше дети играли во дворе. Теперь во дворе играют машины, а дети - в телефоне. Прогресс."
"Мы учим детей не врать. При этом сами говорим в Zoom: 'У меня камера сломалась'."
"Онлайн-урок: ребёнок в пижаме изучает дисциплину. Какую дисциплину в пижаме?"

Твои примеры для вдохновения (адаптируй под вопрос):
{examples}

Ответь в стиле Жванецкого - найди парадокс и вырази через точный образ:
...
Твой ответ в стиле Жванецкого:"""
    ...
    async def generate_humor(self, 
        ...
        try:
            ...
            # Получаем примеры: смешиваем старые и новые онлайн-примеры
            old_examples = get_mixed_examples(topic_category)
            online_examples = get_online_examples(3)
            
            # Форматируем примеры в стиле Жванецкого
            all_examples = []
            for ex in online_examples:
                all_examples.append(f'"{ex["example"]}"')
    ...
    async def _call_claude_haiku(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            response = await self.client.chat(
                messages=messages,
                max_tokens=200,  # Немного больше для философских мыслей
                temperature=1.2,  # Высокая температура для неожиданных поворотов
                top_p=0.98,  # Широкий выбор слов
                frequency_penalty=0.7,  # Сильный штраф за повторения
                presence_penalty=0.5,   # Поощрение оригинальности
