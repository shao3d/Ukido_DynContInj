#!/usr/bin/env python3
"""Запуск теста comparing_dad через песочницу"""
...
def run_comparing_dad_test():
    """Запуск полного диалога comparing_dad"""
    
    # Загружаем сценарий
    with open('test_burned_parent.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    scenario = data['scenarios'][1]  # comparing_dad - второй сценарий
    ...
    for i, message in enumerate(messages, 1):
        ...
        try:
            ...
            # Проверка времени ответа (если есть в metadata)
            response_time = result.get("metadata", {}).get("response_time")
            if response_time:
                dialog_log.append(f"- Время ответа: {response_time:.2f}с")
            
            # Проверки специфичные для comparing_dad
            if "CTA" in result.get("metadata", {}):
                cta_info = result["metadata"]["CTA"]
                dialog_log.append(f"- **CTA:** {user_signal} (органично интегрирован)")
